...
struct FuseSplitGemmConcatIntoReshapeGemmReshape final
    : public PredicateBasedPass {
  ...
  bool patternMatchPredicate(Node* node) override {
    ...
    for (auto input : node->inputs()) {
      ...
      if (unsqueeze_n->kind() != kUnsqueeze) {
        return false;
      }

      auto gemm_n = input->node()->input()->node();
      if (gemm_n->kind() != kGemm) {
        return false;
      }

      auto squeeze_n = gemm_n->inputs()[0]->node();
      if (squeeze_n->kind() != kSqueeze) {
        return false;
      }

      auto split_n = squeeze_n->input()->node();
      if (split_n->kind() != kSplit) {
        return false;
      }
      if (split_n->outputs().size() != node->inputs().size()) {
        return false;
      }
    }
    return true;
  ...
  bool runTransform(Node* n, Graph& graph, NodeDestroyType& destroy_current)
    ...
    if (!split_n->hasAttribute(ksplit)) {
      return false;
    }
    if (std::adjacent_find(
            split_n->is(ksplit).begin(),
            split_n->is(ksplit).end(),
            std::not_equal_to<bool>()) != split_n->is(ksplit).end()) {
      return false;
    };
    if (split_n->is(ksplit)[0] != 1) {
      return false;
    }

    if (split_n->outputs().size() != n->inputs().size()) {
      return false;
    }
    ...
    const float alpha =
        first_gemm_n->hasAttribute(kalpha) ? (float)first_gemm_n->f(kalpha) : 1;
    const float beta =
        first_gemm_n->hasAttribute(kbeta) ? (float)first_gemm_n->f(kbeta) : 1;
    const int trans_a =
        first_gemm_n->hasAttribute(ktransA) ? (int)first_gemm_n->i(ktransA) : 0;
    const int trans_b =
        first_gemm_n->hasAttribute(ktransB) ? (int)first_gemm_n->i(ktransB) : 0;
    ...
    for (size_t i = 0; i < n->inputs().size(); i++) {
      ...
      auto squeeze_n = gemm_n->inputs()[0]->node();
      if (!(squeeze_n->hasAttribute(kaxes) &&
            squeeze_n->is(kaxes).size() == 1)) {
        return false;
      }
      if (!(unsqueeze_n->hasAttribute(kaxes) &&
            unsqueeze_n->is(kaxes).size() == 1)) {
        return false;
      }
      ...
      if (gemm_n->inputs().size() != 3) {
        return false;
      }
      Value* w_v = gemm_n->inputs()[1];
      Value* b_v = gemm_n->inputs()[2];
      if (!(gemm_n->inputs()[0]->has_sizes() && w_v->has_sizes() &&
            b_v->has_sizes())) {
        return false;
      }
    ...
    prev_reshape_n->output()->setSizes(
        std::vector<Dimension>{Dimension(prev_shape_t.int64s()[0]),
    ...
    new_gemm_n->output()->setSizes(std::vector<Dimension>{
        Dimension(prev_shape_t.int64s()[0]), Dimension(w_sizes[!trans_b].dim)});
    ...
    post_reshape_n->output()->setSizes(
        std::vector<Dimension>{Dimension(post_shape_t.int64s()[0]),
